...
const createBulkAttendance = asyncHandler(async (req, res) => {
  ...
  const nonPresent = populated.filter((item) => item.status !== 'present');
...
const getDailyReport = asyncHandler(async (req, res) => {
    ...
    const report = students.map((s) => {
      ...
      if (attendanceRecordedToday) {
        if (records.length === 0) {
          ...
          const incidentRecords = records.filter((r) => r.status !== 'present');
  ...
  const computePeriodCell = (records, period) => {
    ...
    if (statuses.includes('present') && statuses.some((value) => value !== 'present')) {
  ...
  const summary = {
    total: students.length,
    morning: {
      present: report.filter((row) => row.morning.status === 'present').length,
    ...
    afternoon: {
      present: report.filter((row) => row.afternoon.status === 'present').length,
