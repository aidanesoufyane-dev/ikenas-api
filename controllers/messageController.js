...
const buildMessageFilter = async (user) => {
  ...
  if (user.role === 'teacher') {
    ...
    filter.$or = [
      ...
      { targetUser: user.id }, { sender: user.id },
      { sender: user.id },
    ];
    return filter;
  }
  ...
  filter.$or = [
    { recipientType: 'broadcast' },
    ...(student ? [{ recipientType: 'class', targetClass: student.classe }] : []),
    { targetUser: user.id }, { sender: user.id },
  ];
...
const isUserMessageRecipient = async (user, message) => {
  const messageSenderId = String(message.sender?._id || message.sender || '');
  if (messageSenderId === String(user.id)) {
    return false;
  }
  ...
  if (message.recipientType === 'individual') {
    return String(message.targetUser?._id || message.targetUser || '') === String(user.id);
  }
  ...
  if (message.recipientType === 'class') {
    ...
    const student = await Student.findOne({ user: user.id }).select('classe');
    return String(student?.classe || '') === targetClassId;
  }
...
const sendMessage = asyncHandler(async (req, res) => {
  ...
  const allowReplyForRecipients = parseBoolean(allowReply, replyExpiresAt, false);
...
const replyToMessage = asyncHandler(async (req, res) => {
  ...
  const isRecipient = await isUserMessageRecipient(req.user, originalMessage);
  if (!isRecipient) {
  ...
  if (!originalSenderId || originalSenderId === String(req.user.id)) {
  ...
  const populatedReply = await Message.findById(replyMessage._id)
    .populate('sender', 'firstName lastName role avatar')
    .populate('targetClass', 'name')
    .populate('targetUser', 'firstName lastName role avatar');
  ...
  if (req.io && populatedReply?.targetUser) {
    ...
    req.io.to(`user-${req.user.id}`).emit('new-message', populatedReply);
  }
...
const getMessages = asyncHandler(async (req, res) => {
  ...
  const filter = await buildMessageFilter(req.user);
...
const markAsRead = asyncHandler(async (req, res) => {
  const visibilityFilter = await buildMessageFilter(req.user);
