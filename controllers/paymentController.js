...
const searchStudentsForCashier = asyncHandler(async (req, res) => {
  ...
  if (q) {
    ...
    filter.$or = [
      ...
      { matricule: { $regex: safeQ, $options: 'i' } },
    ];
  }
...
const downloadReceipt = asyncHandler(async (req, res) => {
  ...
  const content = [
    'REÇU DE PAIEMENT',
    '-------------------------',
    `Reçu N°: ${paymentItem.receiptNumber}`,
    `Facture N°: ${invoice.invoiceNumber}`,
    `Élève: ${invoice.student.user.firstName} ${invoice.student.user.lastName}`,
    `Type de frais: ${FEE_TYPE_LABEL[invoice.feeType] || invoice.feeType}`,
    `Montant payé: ${paymentItem.amount} MAD`,
    `Mode de paiement: ${paymentItem.method === 'cash' ? 'Cash' : 'Virement bancaire'}`,
    `Date: ${new Date(paymentItem.paidAt).toLocaleString('fr-FR')}`,
    `Référence: ${paymentItem.reference || '-'}`,
    '',
    'Ikenas Gestion',
  ].join('\n');

  res.setHeader('Content-Type', 'text/plain; charset=utf-8');
  res.setHeader('Content-Disposition', `attachment; filename="${paymentItem.receiptNumber}.txt"`);
