...
const getAssignments = asyncHandler(async (req, res) => {
  ...
  if (req.user.role === 'student') {
    const studentForSubs = await Student.findOne({ user: req.user.id });
    if (studentForSubs) {
      const submissions = await Submission.find({ student: studentForSubs._id, assignment: { $in: assignments.map(a => a._id) } });
      const submittedIds = submissions.map(s => s.assignment.toString());
      const assignmentsWithStatus = assignments.map(a => {
        const obj = a.toObject();
        obj.status = submittedIds.includes(a._id.toString()) ? 'done' : 'pending';
        return obj;
      });
