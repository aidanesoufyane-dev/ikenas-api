...
const notifyGradeAdded = async ({ io, teacher, sheetTitle, classeId, subjectId, noteCount, role }) => {
  try {
...
const getClassRanking = asyncHandler(async (req, res) => {
  ...
  if (req.user.role === 'student') {
    const studentProfile = await Student.findOne({ user: req.user.id }).select('_id classe');
    if (!studentProfile) {
      return res.status(404).json({ success: false, message: 'Profil étudiant non trouvé.' });
    }
  ...
  const saveSimpleResults = asyncHandler(async (req, res) => {
    ...
    if (req.user.role === 'teacher') {
      await notifyGradeAdded({
        ...
        classeId: classe,
        subjectId: subject,
        noteCount: filteredResults.length,
        role: 'admin',
      });
      await notifyGradeAdded({
        io: req.io,
